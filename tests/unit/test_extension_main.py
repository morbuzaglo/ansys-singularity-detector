...
main.py is IronPython-flavoured but its config / bridge helpers are plain
Python; the ACT callbacks (run_singularity_study, ...) reference ExtAPI and are
only exercised in tests/mechanical/test_milestone9.py.
"""
...
def _load_main(monkeypatch, tmp_path):
    # main.py does `from visualization import ...` at import -> make sibling
    # modules importable and pin _HERE to a temp dir we control.
...
def test_venv_python_prefers_explicit_then_repo(monkeypatch, tmp_path):
    ...
    # no explicit -> derive from repo/.venv/Scripts/python.exe
...
def test_settings_defaults_and_override(monkeypatch, tmp_path):
    m = _load_main(monkeypatch, tmp_path)
    assert m._settings()["refinements"] == 3
    (tmp_path / "sd_study_settings.json").write_text(json.dumps({"ratio": 0.85}),
                                                    encoding="utf-8")
    s = m._settings()
    assert s["ratio"] == 0.85 and s["refinements"] == 3       # merged with defaults
