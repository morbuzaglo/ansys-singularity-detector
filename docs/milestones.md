...
| 9 | ACT interface | next | The ribbon: one **Run Singularity Study** button + a study-settings wizard (refinements, ratio, quantity) + progress/status. Every callback a one-liner into the tested `devtools/` functions (spec §21). This replaces the manual steps in `docs/TRYING_IT.md` §B. |
