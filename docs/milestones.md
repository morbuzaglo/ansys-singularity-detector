...
| 9 | ACT interface | **DONE (on 252)** | Full PASS 2026-09-02. `extension/main.py` — 3 ribbon buttons (**Run Singularity Study**, **Add Contours**, **Study Settings**), each a one-liner into a tested function (spec §21). `extension/act_study.py` — the mesh-refinement study **in the live session on the user's EXISTING analysis** (BCs/loads untouched, original mesh restored, per-level `level_XX/{level.json,file.rst}`, `study_result.json` schema the external pipeline consumes). The extension (IronPython 2.7, no numpy) **shells out to the repo `.venv`** for `analyze_study`/`build_contours`/`convergence_charts`; `extension_deployer` writes `sd_config.json` (repo + venv path) + `sd_study_settings.json` and deploys `main.py` + every IP2.7 module. Extension deployed to `%APPDATA%\Ansys\v252\ACT\extensions`. `tests/mechanical/test_milestone9.py` (5 passed — in-session loop, restore, per-level preservation, schema-consumable) + 13 unit (deployer 6, main.py bridge 7). Unit suite = 116. `docs/TRYING_IT.md` §B is now one button. |
