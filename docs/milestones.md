...
| `milestone-8` | Convergence charts: `convergence_charts.py` (global + per-region PNG **and** CSV, spec §39) + `extension_deployer.py` (spec §19) + `docs/TRYING_IT.md` |
| `milestone-9` | ACT ribbon: `extension/main.py` (3 buttons) + `extension/act_study.py` (in-session study on the user's existing analysis) + venv subprocess bridge |
...
| 8 | Region clustering + charts | **DONE (on 252)** | Full PASS 2026-09-02. Clustering was already in M6. `devtools/convergence_charts.py` → `<study>/charts/`: **global** (peak σ vs h, peak σ vs element count, max deformation vs h, reaction imbalance vs h; strain energy skipped — null on 252) + **per region** (stress vs h, increment log-log with slope→λ, finite-vs-divergent model-fit overlay, Nodal Fraction vs level, ZZ-lite energy error vs level, evidence-vs-#levels), each PNG **plus** its CSV, indexed by `charts_index.json`. Also **`devtools/extension_deployer.py`** (spec §19 — assembles the ACT layout into `%APPDATA%\Ansys\v252\ACT\extensions`, `--link`/`--uninstall`) and **`docs/TRYING_IT.md`** (headless + in-GUI walkthroughs). Extension deployed to APPDATA. 8 unit + `test_milestone8.py` (4). Unit suite = 109. |
| 9 | ACT interface | **DONE (on 252)** | 2026-09-02, incl. a UI-feedback polish round. `extension/main.py` — 4 ribbon buttons (**Run Singularity Study**, **Restore Original Mesh**, **Study Settings**, **Add Contours**), each `@_guard`-wrapped (binds the engine, turns any error into a readable message box). Settings are a **"Singularity Study" tree object** (`<object>` with `<property>` Details fields: Refinements / Ratio / Confidence threshold / Result quantity; created with defaults if absent; JSON fallback). `extension/act_study.py` — study on the user's EXISTING analysis; temp result objects are `SD_`-prefixed and deleted at the end (spec §9); **by default the model is LEFT meshed + solved at the finest level** (spec §7) and `sd_original_mesh.json` is written for the separate Restore action (no more "everything goes stale"). Modeless status window (`System.Windows.Forms`, `DoEvents` between steps). The extension (IP 2.7, no numpy) shells out to the repo `.venv` for `analyze_study`/`build_contours`/`convergence_charts`; `contour_fields.csv` is read by physical-coordinate nearest lookup. **Fix**: `visualization.add_singularity_contours` used bare `ExtAPI` (not injected into imported modules) → now via `mech_env.G` + `main.py` passes the extension handle + a 1-arg `CreateResultObject` fallback; `main.py._HERE` guarded against `__file__` being undefined. `extension_deployer` deploys `main.py` + all IP2.7 modules + config; `--link` junction for edit-in-place. `tests/mechanical/test_milestone9.py` (5) + 24 unit (deployer 6, main bridge 9, IP2.7 modules 9). Unit suite = 124. |
